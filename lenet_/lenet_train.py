...
def train_val_process():
    ...
    val_loader = DataLoader(
        dataset=val_data, batch_size=64, shuffle=True, num_workers=0
    )
    ###
    ...
    test_loader = DataLoader(
        dataset=test_set, batch_size=1, shuffle=True, num_workers=0
    )
...
def train_model_process(model, train_loader, val_loader, test_loader, epochs):
    ...
    torch.save(
        best_model_wts, f"/Users/jiawen/Desktop/USYD_S2/5328/assignment2/best_model.pth"
    )
...
def plot(train_process, test_process):
    ...
    plt.subplot(2, 2, 3)
    plt.plot(
        test_process["epoch"], test_process.test_loss_all, "ro-", label="test loss"
    )
...
if __name__ == "__main__":
    ...
    train_process, test_process = train_model_process(
        Lenet, train_loader, val_loader, test_loader, 4
    )
